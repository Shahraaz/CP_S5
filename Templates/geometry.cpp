...
namespace Polygonal
{
    ...
    Polygon convexHull(Polygon p, bool removeRedundant)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            while (m > 1 && dcmp(cross(ch[m - 1] - ch[m - 2], p[i] - ch[m - 1])) <= check)
                m--;
            ch[m++] = p[i];
        }
        ...
        for (int i = n - 2; i >= 0; i--)
        {
            while (m > k && dcmp(cross(ch[m - 1] - ch[m - 2], p[i] - ch[m - 2])) <= check)
                m--;
            ch[m++] = p[i];
        }
        if (n > 1)
...
namespace Convex
{
    ...
    {
        return dcmp(cross(b - a, p - a)) >= -EPS && dcmp(cross(c - b, p - b)) >= -EPS && dcmp(cross(a - c, p - c)) >= -EPS;
    }
    ...
    // Tested : https://www.spoj.com/problems/INOROUT
    // pt must be in ccw order
    // returns inside or on = 0, outside = 1
    int pointInConvexPolygonNonCollinear(const Polygon &pt, Point p)
    {
        int n = pt.size();
        assert(n >= 3);
        ...
        for (size_t i = 0; i < pt.size(); i++)
        {
            ...
            if (!pointInTriangle(pt[i], pt[next], pt[next_next], p))
            {
                db(pt[i], pt[next], pt[next_next], p, "intriangle");
                return 1;
            }
            next = next_next;
            next_next++;
            if (next_next >= pt.size())
                next_next -= pt.size();
        }
...
namespace Circular
{
    ...
    Tf circleTriangleIntersectionArea(Circle c, Segment s)
    {
        ...
        return circleTriangleIntersectionArea(c, Segment(s.a, Sect[0])) + circleTriangleIntersectionArea(c, Segment(Sect[0], Sect[1])) + circleTriangleIntersectionArea(c, Segment(Sect[1], s.b));
    }
    ...
    pair<int, int> anyIntersection(const vector<Segment> &v)
    {
        ...
        for (auto &cur : ev)
        {
            ...
            if (cur.tp == 1)
            {
                ...
                iter pre = prev(nxt);
                if (pre != st.end() && segmentsIntersect(v[*pre], v[id]))
                    return {*pre, id};
                if (nxt != st.end() && segmentsIntersect(v[*nxt], v[id]))
                    return {*nxt, id};
...
vector<Point> intersect_polygon(const vector<Point> &p1, const vector<Point> &p2)
{
    vector<Point> points;
    for (size_t i = 0; i < p1.size(); ++i)
    {
        if (Convex::pointInConvexPolygonNonCollinear(p2, p1[i]) == 0)
        {
            db(p1[i].x, p1[i].y, "inside p2");
            points.emplace_back(p1[i]);
        }
        for (size_t j = 0; j < p2.size(); ++j)
        {
            // Add points of intersection.
            Point ret;
            if (Linear::lineLineIntersection(p1[i], p1[(i + 1) % p1.size()], p2[j], p2[(j + 1) % p2.size()], ret))
            {
                db("intersection", i, j, ret.x, ret.y);
                points.pb(ret);
            }
        }
    }
    ...
    for (size_t i = 0; i < p2.size(); ++i)
        if (Convex::pointInConvexPolygonNonCollinear(p1, p2[i]) == 0)
        {
            db(p2[i].x, p2[i].y, "inside p1");
            points.emplace_back(p2[i]);
        }
    db("raw");
    print_polygon(points);
    points = Polygonal::convexHull(points, true);
    db("compressed");
    print_polygon(points);
    return Polygonal::convexHull(points, true);
}
...
void solveCase()
{
    ...
    vector<Point> polygon_prev;
    for (size_t i = 0; i < n; i++)
    {
        int c;
        cin >> c;
        vector<Point> polygon_curr;
        ...
        db(i, "polygon");
        Polygonal::RemoveCollinear(polygon_curr);
        print_polygon(polygon_curr);
        if (i == 0)
        {
            polygon_prev = polygon_curr;
            continue;
        }
        polygon_prev = intersect_polygon(polygon_prev, polygon_curr);
        Polygonal::RemoveCollinear(polygon_prev);
    }
    cout << fixed << setprecision(10);
    cout << Polygonal::signedPolygonArea(polygon_prev) << '\n';
}
