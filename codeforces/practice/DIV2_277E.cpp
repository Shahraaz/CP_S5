...
int minLen[NAX], cntSelectedOfLen[NAX];
bool selected[NAX];
class Solution
{
private:
public:
    ...
    void solveCase()
    {
        ...
        for (int i = n - 1; i >= 0; i--)
        {
            auto next = S.query(a[i] + 1, NAX - 1);
            lisLen[i] = next.val + 1;
            S.update(a[i], Node(lisLen[i]));
        }
#ifdef LOCAL
...
#endif
        int lisLength = *max_element(all(lisLen));
        fill(minLen, minLen + NAX, MOD);
        minLen[lisLength + 1] = -MOD;
        for (size_t i = 0; i < n; i++)
        {
            if (minLen[lisLen[i] + 1] < a[i])
            {
                selected[i] = true;
                minLen[lisLen[i]] = min(minLen[lisLen[i]], a[i]);
                cntSelectedOfLen[lisLen[i]]++;
            }
            db(i, selected[i]);
        }
        for (size_t i = 0; i < n; i++)
        {
            ...
            {
                if (cntSelectedOfLen[lisLen[i]] > 1)
                    cout << 2;
                else
                    cout << 3;
            }
